...
$sql = "SELECT id, nombre, precio, stock, descripcion, img FROM productos";
$resultado = $conn->query($sql);

$sql2 = "SELECT img FROM productos";
$result = $conn->query($sql2);

// Array para almacenar las URLs de las imágenes
$image_urls = array();

// Recuperar las URLs y almacenarlas en el array
...
<body>
    <header>
                ...
                <ul class="navbar-nav ml-auto margin-r">
                    ...
                    <ul>
                        <li class="nav-item dropdown">
                            <a class="nav-link dropbtn active" href="tienda.php">TIENDA<img class="img-tienda"
        ...
        <div class="margin-t buscar" id="buscar">
            ...
            <div class="row top">
                ...
                if ($resultado->num_rows > 0) {
                    while ($row = $resultado->fetch_assoc()) {
                        // Calculate the discounted price inside the loop
                        $precioConDescuento = $row['precio'] * (1 - $descuento);
                        // Obtener las URLs de las imágenes del producto actual
                        $img = explode(',', $row['img']);
                        $active = true; // Variable para controlar el estado activo del primer ítem del carrusel
                        echo '<div class="col-md-4 justify-content-center">';
                        echo '<div class="card mb-4">';
                        // Mostrar el carrusel de imágenes para este producto
                        echo '<div id="carousel_' . $row['id'] . '" class="carousel slide" data-ride="carousel">';
                        echo '<div class="carousel-inner">';
                        foreach ($img as $img_url) {
                            echo '<div class="carousel-item ' . ($active ? 'active' : '') . '">';
                            echo '<img src="' . $img_url . '" class="card-img-top" alt="Producto">';
                            echo '</div>';
                            $active = false; // Cambiar el estado activo después del primer ítem
                        }
                        echo '</div>';
                        echo '<a class="carousel-control-prev" href="#carousel_' . $row['id'] . '" role="button" data-slide="prev">';
                        echo '<span class="carousel-control-prev-icon" aria-hidden="true"></span>';
                        echo '<span class="sr-only">Previous</span>';
                        echo '</a>';
                        echo '<a class="carousel-control-next" href="#carousel_' . $row['id'] . '" role="button" data-slide="next">';
                        ...
                        echo '<p class="card-text"><b>' . number_format($precioConDescuento, 2) . '€</b>';
                        if ($descuento > 0) {
                            echo ' <span style="color: red; font-size: 12px;">-10%</span>';
                        }
                        echo '</p>';
                        echo '<a href="producto.php?id=' . $row['id'] . '" class="button">Ver Producto</a>';
                        echo '</div>';
                        echo '</div>'; // Cierre de la tarjeta
                        echo '</div>'; // Cierre del contenedor de la columna
                    }
                } else {
